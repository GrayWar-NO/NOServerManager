...
deployment. All other parts of the suite can continue running without the central, but the integrity of the logs is not
...
The last entry in the provided config, `aircraft`, can be omitted if you're only running vanilla servers. For modded,
you should add the chat names of your modded planes to the default provided value list.
...
there to configure it). Once that is done, create a server certificate with `newServerCert.sh`.
...
4. Grab a copy of the [GrayWar server plugin](https://github.com/GrayWar-NO/GrayWar-Server-Plugin). You might need to
   ...
   `main`.
5. Run your server once to generate the config. It will be located in `$SERVER_DIR/config/BepInEx/config`. You will then
   need to [configure the server](#configuring-a-server).
...
7. Go back to your game server, and place a copy of `ca.crt`, `servername.crt` and `servername.key` in the `CA` folder
   (they will be generated by the previous step in your central's `CA` folder.)
8. `docker compose up -d`
9. enjoy!

## Configuring a server

The GrayWar server plugin configuration is quite long and extensive. You can look through it and configure it as you
wish. Here are the required config options to start a new server.

Scroll to the gRPC interface section. `server name` is the name that your server will appear as in the DB, and the name
of the certificate files it will look for.
